...
uint32_t synth(uint32_t sgn, uint32_t ex, uint32_t man) {
  return (sgn << 31) | (ex << 23) | man;
}

uint64_t maninv_22downto2(uint32_t man_geta) {
  uint32_t manans = 0;
  uint32_t man[22];
...
uint64_t maninv_22downto1(uint32_t man_geta) {
...
uint64_t maninv_22downto0(uint32_t man_geta) {
...
uint32_t manulp(uint64_t cat, uint32_t man_geta, int i) { // rounding (round even) or carry
  uint32_t man, manans;
  manans = cat & 0xffffff;
  man = cat >> 32;
...
uint32_t maninv_sub0(uint32_t man_geta) {
  return manulp(maninv_22downto2(man_geta), man_geta, 2) >> 2;
}

uint32_t maninv_sub1(uint32_t man_geta) {
  return manulp(maninv_22downto1(man_geta), man_geta, 1) >> 1;
}

uint32_t maninv_norm(uint32_t man_geta) {
